...
class DataFormasi extends BaseController 
{
	...
	public function view($idInstansi)
	{
		...
		$cekFakultas = $this->M_instansi->where('instansi_id', $idInstansi)->first();
		$data ['instansi_nama']   = $cekFakultas['instansi_nama'];
		$data ['id_instansi'] = $cekFakultas['instansi_id'];

		//Jika Data fakultas ada
		if ($cekFakultas) {
			return view('v_dataFormasi/view', $data);
		} else {
		 	return view('v_dataFormasi/error', $data);
		}
